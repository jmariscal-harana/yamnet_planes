...
path_data_train = os.path.join(path_root, 'Datasets/small_aircraft_v2/training_data/')
path_data_csv = os.path.join(path_root, 'Datasets/small_aircraft_v2/data_csv/')
...
path_data_csv_file = os.path.join(path_data_csv, 'audio_paths_20201214_205907.csv')
...
if feature_extraction_method == 1:
    df_data_csv = pd.read_csv(path_data_csv_file)
...
# OPTION 2: SAVE INDIVIDUAL FEATURES
# Load all features and labels and save them into the same vector
    ...
    for class_idx, class_label in enumerate(classes):
        ...
        # Load features and labels for each audio and append
        for idx, path_audio in enumerate(tqdm(path_audios)):
            ...
            for idx_aug in range(num_augmentations[class_idx] + 1):
                ...
                path_labels_tmp = path_labels + '_{:02d}'.format(idx_aug) + '.npy'
                
                if os.path.isfile(path_features_tmp) and os.path.isfile(path_labels_tmp):
                    features_tmp = np.load(path_features_tmp)
                    labels_tmp = np.load(path_labels_tmp)

                    for feature, label in zip(features_tmp, labels_tmp):
                        features.append(feature)
                        labels.append(label)

                else:
                    print('WARNING: Cannot load file {}; it does not exist.'.format(audio_filename))
